...
# module "kubectl-yaml-autopilot-jwt" {
#    source = "terraform-google-modules/gcloud/google//modules/kubectl-wrapper"
#    project_id              = data.google_project.project_autopilot.project_id
#    cluster_name            = google_container_cluster.primary_autopilot.name
#    cluster_location        = var.region
#    module_depends_on       = [google_container_cluster.primary_autopilot]
...
# module "kubectl-yaml-autopilot-boa" {
#    source = "terraform-google-modules/gcloud/google//modules/kubectl-wrapper"
#    project_id              = data.google_project.project_autopilot.project_id
#    cluster_name            = google_container_cluster.primary_autopilot.name
#    cluster_location        = var.region
#    module_depends_on       = [module.kubectl-yaml-autopilot-jwt]
...
resource "google_container_registry" "registry_autopilot" {
  project = data.google_project.project_gke.project_id
  location = "EU"
}

resource "google_storage_bucket_iam_member" "autopilot_viewer" {
  bucket = google_container_registry.registry_autopilot.id
  role = "roles/storage.objectViewer"
  member = google_service_account.default_autopilot.member
}

resource "google_monitoring_dashboard" "dashboard_pilot" {
  project = data.google_project.project_autopilot.project_id
  dashboard_json = <<EOF
{
  "displayName": "Bank of Anthos Dashboard - Autopilot",
  "gridLayout": {
    "columns": "2",
    "widgets": [
      {
